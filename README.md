...
→ 等待审计服务健康（确认请求校验器与扫描引擎两个组件就绪）→ API/HTTP
冒烟：高密度两簇重复框同竖线部分交接（面积 200、周长 70，公共段内化、
上下外露竖边保留）、同横坐标完全相接（周长 60）与不相交交接
（面积 100、周长 60）、重叠框面积 10 周长 14、相邻方框不计公共边、
重复框不增量、重复标识/非法矩形带位置拒绝且无部分结果，随后退出并以
退出码报告结果。
